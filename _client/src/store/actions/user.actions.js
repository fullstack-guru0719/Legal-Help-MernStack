        ...
        case 'firebase':
            {
                firebaseService.updateUserData(user)
                    .then(() => {
                        store.dispatch(messageActions.showMessage({ message: "User data saved to firebase" }));
                    })
                    .catch(error => {
                        store.dispatch(messageActions.showMessage({ message: error.message }));
                    });
                break;
            }
        case 'auth0':
            {
                auth0Service.updateUserData({
                    ...
                    .then(() => {
                        store.dispatch(messageActions.showMessage({ message: "User data saved to auth0" }));
                    })
                    .catch(error => {
                        store.dispatch(messageActions.showMessage({ message: error.message }));
                    });
        ...
        default:
            {
                jwtService.updateUserData(user)
                    .then(() => {
                        store.dispatch(messageActions.showMessage({ message: "User data saved with api" }));
                    })
                    .catch(error => {
                        store.dispatch(messageActions.showMessage({ message: error.message }));
                    });
